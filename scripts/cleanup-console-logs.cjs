 ...
 * Usage: node scripts/cleanup-console-logs.cjs [--dry-run]
 */
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!isComment && !isStringLiteral && /console\.log\s*\(/.test(line)) {
                ...
                console.log(`    Line ${i + 1}: ${trimmed.substring(0, 80)}${trimmed.length > 80 ? '...' : ''}`)
        ...
        if (modified) {
            ...
            if (!isDryRun) {
                ...
                console.log(`  ✓ Cleaned: ${path.relative(process.cwd(), filePath)} (removed ${removedCount} console.log)\n`)
            } else {
                console.log(`  [DRY RUN] Would remove ${removedCount} console.log statements\n`)
            }
        }
    } catch (error) {
...
function main() {
    ...
    console.log(`  Files with issues:   ${filesModified}`)
    console.log(`  console.log found:   ${consoleLogsRemoved}`)
    console.log(`  Duration:            ${duration}s`)
    console.log('='.repeat(60))

    if (isDryRun) {
        console.log('\n⚠️  This was a DRY RUN. Run without --dry-run to apply changes.')
        console.log('Command: node scripts/cleanup-console-logs.cjs\n')
    } else {
        console.log('\n✅ Cleanup complete!\n')
    }
