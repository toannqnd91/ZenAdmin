...
export const useProductsCategoriesTable = async () => {
  ...
  // Computed
  const categories = computed(() => (data.value as ProductCategoriesApiResponse)?.data || [])

  const filtered = computed(() =>
    categories.value.filter((cat: ProductCategory) => cat.name.toLowerCase().includes(q.value.toLowerCase()))
  )
