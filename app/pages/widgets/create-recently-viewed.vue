...
<template>
  <UDashboardPanel class="flex flex-col h-full">
    <template #header>
      <UDashboardNavbar title="Create Recently Viewed Widget">
        <template #leading>
    ...
    <template #body>
      <UCard class="w-full mt-6">
        <UForm @submit="onSave" class="space-y-6">
          <div class="text-3xl font-light mb-8">Create Recently Viewed Widget</div>
          ...
          <div class="grid grid-cols-12 gap-4 items-center mb-2">
            <label class="col-span-2 text-right pr-2">Item Count</label>
            <div class="col-span-10 w-full">
              <UInput v-model="itemCount" type="number" min="1" placeholder="Enter item count" class="w-full" />
            </div>
          </div>
