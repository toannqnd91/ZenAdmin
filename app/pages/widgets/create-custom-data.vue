...
import type { CreateCustomDataWidgetRequest } from '@/services/widgets.service'
...
onMounted(async () => {
  ...
  if (response.success && response.data) {
    ...
    if (!widgetZone.value && widgetZoneItems.value.length > 0) {
      const first = widgetZoneItems.value[0]
      if (first) widgetZone.value = first.value
    }
  }
})
...
// removed validateJson (no longer needed)
...
async function onSave() {
  ...
  try {
    const toIso = (val: string) => val
      ? new Date(val.replace(/(\d{2})\/(\d{2})\/(\d{4}) (\d{2}):(\d{2})/, '$3-$2-$1T$4:$5:00')).toISOString()
      : null

    const payload: CreateCustomDataWidgetRequest = {
      id: 0,
      ...
      items: items.value.map(i => ({
        ...
        subCaption: i.subCaption.trim(),
        imageUrl: i.imageUrl,
        linkText: i.linkText.trim(),
        targetUrl: i.targetUrl.trim(),
    ...
    alert('Có lỗi khi tạo widget! ' + msg)
  } finally {
...
function handleFileChange(e: Event, idx: number) {
  ...
  reader.onload = (ev) => {
    ...
    if (typeof result === 'string') items.value[idx].imageUrl = result
  }
          ...
          <div class="grid grid-cols-12 gap-4 items-start mb-2">
            ...
            <div class="col-span-10 w-full">
              ...
              <UButton
                ...
                size="xs"
                class="mt-2"
                @click="() => { items.push({ caption: '', subCaption: '', imageUrl: '', linkText: '', targetUrl: '', sortOrder: items.length }) }"
              >
                Add more
              </UButton>
            </div>
