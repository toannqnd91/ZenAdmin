...
async function fetchData() {
    ...
    try {
        const res = await newsCategoryService.getCategory(categoryId)
        if (res.success && res.data) {
            state.name = res.data.name
            state.description = res.data.description || ''
            state.parentId = res.data.parentId || null
            state.imageUrl = res.data.imageUrl || ''
            state.icon = res.data.icon || ''
            state.isVisible = res.data.isShow ?? true
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    try {
        ...
        const res = await newsCategoryService.updateCategory(categoryId, payload)

        if (res.success) {
            toast.add({
                title: 'Thành công',
                description: 'Cập nhật danh mục tin tức thành công',
                color: 'success'
            })
            router.push('/news-categories')
        } else {
        ...
        console.error('Update news category error:', e)
    } finally {
...
<template>
    <UDashboardPanel>
        <template #header>
            <UDashboardNavbar title="Chỉnh sửa danh mục tin tức">
                <template #leading>
