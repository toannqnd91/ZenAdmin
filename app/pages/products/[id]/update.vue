...
// Load detail and map to create-page structures
const loadDetail = async () => {
  ...
  const data: ProductDetail = res?.data as ProductDetail
    if (data) {
  // Set id on form for update mode
  page.state.formData.id = data.id || productId
      // Set brand selection if present (brandId from API)
      if (data.brandId) {
        page.state.trademark = data.brandId
      } else {
        page.state.trademark = null
      }

      // Set supplier selection if present (supplierId from API)
      if (data.supplierId) {
        page.state.selectedSupplier = data.supplierId
      } else {
        page.state.selectedSupplier = null
      }
  ...
  const imgs = Array.isArray(data.productImages) ? data.productImages.map((p: any) => p.mediaUrl).filter(Boolean) : []
  ...
  const opts = Array.isArray(data.options) ? data.options : []
      ...
      for (const o of opts) {
        page.variants.attributes.push({ name: o.name, values: (o.values || []).map(v => v.key), input: '' })
      }
...
<template>
  ...
  <UDashboardPanel id="update-product">
      ...
      <UDashboardNavbar>
        <template #leading>
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-lg font-semibold">Cập nhật sản phẩm</div>
            </div>
          </div>
        </template>
        <template #right>
    ...
    <template #body>
      <div v-if="loadingDetail" class="py-10 text-center text-gray-500">Đang tải...</div>
      <div v-else-if="fetchError" class="py-10 text-error">{{ fetchError }}</div>
          ...
          <div class="flex-1 space-y-6">
            ...
            <UPageCard title="Giá bán" variant="soft" class="bg-white rounded-lg">
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="ml-1" title="Giá so sánh là giá gốc trước khi giảm giá.">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" /></svg>
                      </span>
                    </div>
                    <div class="relative">
                ...
                <div class="flex items-center mt-4">
                  <CustomCheckbox class="mr-2" :model-value="!!page.state.formData.chargeTax" @update:model-value="page.state.formData.chargeTax = $event">
                ...
                <div class="flex flex-col md:flex-row gap-4">
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="ml-1" title="Giá nhập là giá bạn mua vào."><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" /></svg></span>
                    </div>
                    <div class="relative">
                      <input id="importPrice" v-model.number="page.state.formData.importPrice" type="number" min="0" step="0.01" placeholder="0.00" class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-12">
                      <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-base pointer-events-none">đ</span>
                    </div>
                  </div>
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      ...
                      <div class="flex items-center gap-1"><span class="text-gray-500 text-xs" title="Lợi nhuận = Giá bán - Giá nhập"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" /></svg></span></div>
                    </div>
                    <div class="relative">
                      <input id="profit" :value="page.inventory.profitDisplay" readonly class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-12">
                      <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-base pointer-events-none">đ</span>
                    </div>
                  </div>
                  <div class="flex-1">
                    ...
                    <div class="relative">
                      <input id="margin" :value="page.inventory.marginDisplay" readonly class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-8">
            ...
            <UPageCard title="Tồn kho" variant="soft" class="bg-white rounded-lg">
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                <div class="space-y-4">
                  <div class="pt-1">
                    <CustomCheckbox :model-value="!!page.state.formData.manageInventory" @update:model-value="page.state.formData.manageInventory = $event"><span class="text-gray-900">Quản lý số lượng tồn kho</span></CustomCheckbox>
                  </div>
                  <div class="pt-1">
                  ...
                  <div v-if="page.state.formData.manageInventory">
                    <CustomCheckbox class="items-start" :model-value="!!page.state.formData.allowNegativeStock" @update:model-value="page.state.formData.allowNegativeStock = $event">
                      <span class="text-gray-900">Cho phép bán khi hết hàng<div v-if="page.state.formData.allowNegativeStock" class="text-gray-500 text-xs">Nhân viên vẫn có thể bán khi tồn kho bằng 0 hoặc âm. (Không ảnh hưởng đến POS)</div></span>
                    </CustomCheckbox>
                  </div>
                  <div class="space-y-2">
                    ...
                    <div v-if="page.state.formData.hasSkuOrBarcode" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input v-model="page.state.formData.sku" type="text" class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500" placeholder="Nhập mã SKU">
                      </div>
                      <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Mã vạch (ISBN, UPC, GTIN, ...)</label>
                        <input v-model="page.state.formData.barcode" type="text" class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500" placeholder="Nhập mã vạch">
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      <template v-for="(img, idx) in page.media.imagePreviews" :key="idx">
                        <div :class="['relative flex items-center justify-center border border-gray-300 rounded-md bg-white overflow-hidden', idx === 0 ? 'row-span-2 col-span-2 h-52 w-full' : 'h-24 w-full']" draggable="true" @dragstart="(e) => page.media.onDragStart(e, idx)" @dragover.prevent="page.media.onDragOver" @drop="(e) => page.media.onDrop(e, idx)" @dragend="page.media.onDragEnd">
                          ...
                          <div v-else-if="page.media.isUploadingImage" class="flex flex-col items-center justify-center w-full h-full">
                            <svg class="animate-spin h-8 w-8 text-gray-400 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" /><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>
            ...
            <UPageCard variant="soft" class="bg-white rounded-lg">
              <div class="flex items-center justify-between">
                <div class="font-semibold text-base">Thuộc tính</div>
                <button v-if="!page.variants.showAttributesEditor" type="button" class="text-primary-600 hover:underline" @click="page.variants.startAttributes">Thêm thuộc tính</button>
              </div>
              <div v-if="!page.variants.showAttributesEditor" class="-mx-6 px-6 pt-2 pb-4 border-t-1 border-gray-200 dark:border-gray-700 flex flex-col justify-center min-h-[64px]">
              ...
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                ...
                <div class="flex flex-wrap items-center gap-3 mb-3">
                  ...
                  <template v-for="(a, i) in page.variants.activeAttributes" :key="i">
                    <button type="button" class="inline-flex items-center gap-1 text-sm text-primary-600 bg-primary-50 px-2 py-1 rounded-md hover:underline">{{ a.name }}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="h-4 w-4"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 9l6 6 6-6" /></svg></button>
                  </template>
                </div>
                <div class="flex items-center gap-2 py-2 border-y border-gray-200">
                  ...
                  <div v-if="page.variants.selectedVariantKeys.length > 0" class="ml-auto">
                    <UDropdownMenu :items="[{ label: 'Chỉnh sửa giá', value: 'edit-price' }, { label: 'Chỉnh sửa tồn kho', value: 'edit-stock' }, { label: 'Chỉnh sửa SKU', value: 'edit-sku' }, { label: 'Chỉnh sửa giá vốn', value: 'edit-cost' }, { label: 'Xoá phiên bản', value: 'delete' }, { label: 'Chỉnh sửa hàng loạt', value: 'bulk-edit' }]">
                      <UButton variant="soft" size="sm" class="flex items-center gap-1">Chỉnh sửa<svg class="ml-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg></UButton>
                    </UDropdownMenu>
                  </div>
                </div>
                <div>
                  <div v-for="(v, vi) in page.variants.variants" :key="v.key || vi" class="flex items-center justify-between py-4 border-b border-gray-100">
                    <div class="flex items-center gap-3">
                      <CustomCheckbox :model-value="page.variants.selectedVariantKeys.includes(v.key)" @update:model-value="page.variants.onToggleVariant(v.key, $event)" />
                    ...
                    <div class="text-right text-sm leading-5">
                      <div>
                        ...
                        <template v-else-if="page.variants.isEditingPrice(v.key)">
                          <input ref="el => priceInputs[vi] = el as HTMLInputElement" type="number" min="0" class="w-24 px-1 py-0.5 border border-gray-300 rounded text-right text-gray-900" :value="v.price" @blur="page.variants.commitPrice(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))" @keydown.enter.prevent="page.variants.commitPrice(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))">
                        </template>
                      </div>
                      <div class="mt-1">
                        ...
                        <template v-else-if="page.variants.isEditingStock(v.key)">
                          <input ref="el => stockInputs[vi] = el as HTMLInputElement" type="number" min="0" class="w-20 px-1 py-0.5 border border-gray-300 rounded text-right text-gray-900" :value="v.stock" @blur="page.variants.commitStock(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))" @keydown.enter.prevent="page.variants.commitStock(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))">
                        </template>
            ...
            <div class="w-full flex justify-end mt-8 mb-8">
              <UButton label="Lưu sản phẩm" color="primary" size="lg" :loading="page.state.productForm.isSubmitting" class="px-8 py-2 text-lg font-semibold rounded-md shadow" @click="onSubmit" />
            </div>
          </div>
