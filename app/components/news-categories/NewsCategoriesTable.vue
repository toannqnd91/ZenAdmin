...
interface Props {
    ...
    getRowItems: (row: Row<NewsCategory>) => unknown[]
}
...
const columns: TableColumn<NewsCategory>[] = [
    {
        id: 'select',
        header: ({ table }) => {
            console.log('Rendering select header checkbox', table)
            return h(UCheckbox, {
        ...
        cell: ({ row }) => {
            console.log('Rendering select cell checkbox for row:', row.original)
            return h(UCheckbox, {
...
<template>
    <div class="flex flex-col h-full">
        ...
        <div class="flex flex-wrap items-center justify-between gap-1.5 mb-4">
            <UInput :model-value="q" placeholder="Tìm kiếm danh mục..." icon="i-lucide-search" class="max-w-sm"
                @update:model-value="$emit('update:q', $event)" />

            <div class="flex flex-wrap items-center gap-1.5">
                <NewsCategoriesDeleteModal :count="(table as any)?.tableApi?.getFilteredSelectedRowModel().rows.length">
        ...
        <!-- Data Table -->
        <UTable ref="table" :model-value="rowSelection" :pagination="pagination" :data="filtered" :columns="columns"
            :loading="loading" class="shrink-0 flex-1" :ui="{
