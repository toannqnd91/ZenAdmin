...
<template>
  <BaseModal :model-value="open" title="Hủy đơn hàng" width-class="max-w-lg" @update:model-value="(v) => open = v">
    <div class="space-y-6">
      ...
      <div v-if="isPaid" class="space-y-3">
        ...
        <div class="space-y-2">
          <label class="flex items-center gap-3 cursor-pointer">
            <input v-model="refundOption" type="radio" :value="RefundOptionEnum.RefundNow"
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            <input v-model="refundOption" type="radio" :value="RefundOptionEnum.RefundLater"
              class="w-5 h-5 text-primary-600 border-gray-300 focus:ring-primary-500">
      ...
      <div class="space-y-3">
        <label class="flex items-center gap-3 cursor-pointer">
          <input v-model="restockItems" type="checkbox"
            class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
          <span class="text-sm text-gray-700">Hoàn kho tất cả sản phẩm</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer">
          <input v-model="deleteOrder" type="checkbox"
            class="w-5 h-5 text-red-600 border-gray-300 rounded focus:ring-red-500">
          <span class="text-sm text-red-600 font-medium">Xóa đơn</span>
        </label>
      </div>
      ...
      <div class="space-y-2">
        ...
        <AsyncSelect v-model="selectedReason" :fetch-fn="fetchReasons" label-field="label"
          :get-item-key="(item: any) => item.value.toString()" placeholder="Chọn lý do" :searchable="false"
          :clearable="false" :full-width="true" :append-to-body="true" />
      </div>
