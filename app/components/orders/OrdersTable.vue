...
const columns: TableColumn[] = [
  ...
  { key: 'source', label: 'Nguồn đơn', sortable: true, align: 'right' },
  { key: 'total', label: 'Thành tiền', sortable: true, align: 'right' },
  { key: 'paymentStatus', label: 'Trạng thái thanh toán', sortable: true, align: 'right' },
  { key: 'processStatus', label: 'Trạng thái xử lý', sortable: true, align: 'right' }
]
...
<template>
  <BaseTable :data="props.data" :loading="props.loading" :q="props.q" :row-selection="props.rowSelection"
    :pagination="props.pagination" :columns="columns" :col-widths="colWidths" title="Đơn hàng" :tabs="props.tabs"
    tabs-style="underline" :total-pages="props.totalPages" :total-records="props.totalRecords" :show-row-actions="false"
    :disable-client-filter="true" @update:q="val => emit('update:q', val)"
    @update:row-selection="val => emit('update:rowSelection', val)"
    @update:pagination="val => emit('update:pagination', val)" @update:tab="val => emit('update:tab', val)">
    <!-- Custom column render: total (currency) -->
    <template #column-code="{ value, item }">
      <div class="flex flex-col">
        <NuxtLink :to="`/orders/${String(value).replace(/^#/, '')}`"
          class="text-primary-600 hover:underline font-medium inline-block truncate align-middle"
          :title="String(value)">
          {{ value }}
        </NuxtLink>
        <span class="text-xs text-gray-500 truncate" :title="(item as any).customer">
    ...
    <template #column-paymentStatus="{ value }">
      <span class="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium select-none"
        ...
        <template v-else-if="isOverPaid(value as string)">
          ...
          <svg class="w-3.5 h-3.5 text-violet-600" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        ...
        <span v-else class="inline-block h-2 w-2 rounded-full" :class="paymentStatusDot(value as string)" />
        {{ value }}
      </span>
    </template>
    ...
    <template #column-processStatus="{ value }">
      <span class="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium select-none"
        ...
        <span class="inline-block h-2 w-2 rounded-full" :class="processStatusDot(value as string)" />
        {{ value }}
      </span>
    </template>
  </BaseTable>
