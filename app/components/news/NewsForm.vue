...
const isEdit = computed(() => typeof props.newsId === 'number' && !isNaN(props.newsId))

// Use composable (supports optional id for edit mode)
const {
  formData,
  isSubmitting,
  isLoading,
...
<template>
  <UDashboardPanel :id="isEdit ? 'edit-news' : 'add-news'">
    <template #header>
        ...
        <nav class="mb-6" aria-label="Breadcrumb">
          <ol class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <li class="flex items-center">
              ...
              <span class="text-gray-900 dark:text-white font-medium">{{ isEdit ? 'Chỉnh sửa tin tức' : 'Thêm tin tức mới' }}</span>
            </li>
          </ol>
        </nav>
                  ...
                  <div class="relative dropdown-container">
                    ...
                    <div v-if="isDropdownOpen" class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md">
                      ...
                      <div class="max-h-40 overflow-y-auto">
                        <div v-if="isCategoriesLoading" class="p-3 text-sm text-gray-500">
                          Đang tải danh mục...
                        </div>
                        <div v-else-if="categoriesError" class="p-3 text-sm text-red-500">
                          Lỗi tải danh mục
                        </div>
...
.dark .upload-area { border: 1px dashed #4b5563 !important; }
</style>
