...
<template>
  <BaseTable :q="q" :row-selection="rowSelection" :data="data as unknown as Record<string, unknown>[]"
    :loading="loading" title="Danh sách widget" :columns="columns" :col-widths="colWidths"
    :add-button-dropdown-items="addButtonDropdownItems" search-placeholder="Tìm kiếm widget..." :draggable="true"
    drag-handle-class="drag-handle" :drag-animation="200" @update:q="emit('update:q', $event)"
    @update:row-selection="emit('update:rowSelection', $event)" @reorder="(newOrder) => emit('reorder', newOrder)">
    ...
    <template #row-actions="{ item }">
      <div class="flex items-center space-x-6 justify-end">
        <template v-for="action in props.getRowItems({ original: item as unknown as WidgetInstance }) as any[]"
          :key="action.label">
