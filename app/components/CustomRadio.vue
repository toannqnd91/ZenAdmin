...
const sizeClasses = computed(() => {
  switch (props.size) {
    case 'xs':
      return { outer: 'w-3.5 h-3.5', dot: 'w-1.5 h-1.5' }
    case 'sm':
      return { outer: 'w-4 h-4', dot: 'w-2 h-2' }
    case 'lg':
      return { outer: 'w-5.5 h-5.5', dot: 'w-3 h-3' }
    default:
      // md
...
<template>
  <label :class="['inline-flex items-center gap-2.5 select-none', disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer']">
    <input
      class="peer sr-only"
      type="radio"
      :name="name"
    ...
    <span
      :class="[
        'inline-flex items-center justify-center rounded-full border transition focus:outline-none',
        sizeClasses.outer,
        colorClasses.border,
        colorClasses.ring,
    ...
    <span class="text-sm leading-[1.2] flex items-center">
      <slot>{{ label }}</slot>
