...
const loadDetail = async () => {
  ...
  try {
    ...
    if (data) {
      ...
      page.media.imagePreviews.splice(0, page.media.imagePreviews.length, ...imgs.map((f: string) => {
        if (!f) return ''
        if (f.startsWith('http') || f.includes('/')) return f
        const base = imageBaseUrl || ''
        return base ? `${base}/image/${f}` : `/image/${f}`
      }))
  ...
  <UDashboardPanel :id="isEdit ? 'update-product' : 'add-product'">
    ...
    <template #body>
      ...
      <div v-else class="w-full max-w-6xl mx-auto px-4 lg:px-6">
        ...
        <div v-if="showCreatedBanner"
          class="relative mb-4 rounded-md bg-emerald-50 text-emerald-800 border-0 border-t-4 border-t-emerald-500 p-4">
          ...
          <div class="pl-7 mt-3">
            <UButton label="Thêm sản phẩm khác" color="success" variant="soft"
        ...
        <div class="flex flex-col lg:flex-row gap-6">
          ...
          <div class="flex-1 space-y-6">
            ...
            <UPageCard title="Thông tin sản phẩm" variant="soft" class="bg-white rounded-lg">
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                <form class="space-y-4" @submit.prevent="onSubmit">
                  <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tên sản
                      phẩm</label>
                  ...
                  <div>
                    ...
                    <BaseDropdownSelect v-model="page.state.trademark"
                      :options="(page.state.brands || []).map(b => ({ id: b.id, label: b.name }))"
                      :loading="page.state.brandsLoading" placeholder="Chọn thương hiệu"
                    ...
                    <div v-if="page.state.brandsError" class="text-xs text-red-500 mt-1">Lỗi tải thương hiệu: {{
                      page.state.brandsError }}</div>
                  </div>
                  <div>
                    ...
                    <BaseDropdownSelect v-model="page.state.formData.categoryIds" :options="page.state.categoryOptions"
                      ...
                      class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                    <div v-if="page.state.selectedCategories && page.state.selectedCategories.length > 0"
                      ...
                      <span v-for="category in page.state.selectedCategories" :key="category.id"
                        ...
                        <button type="button"
                          ...
                          @click="page.actions.removeCategory(category.id)">×</button>
                      </span>
                    </div>
                    <p v-if="page.state.errors.categoryIds" class="mt-1 text-sm text-red-600">{{
                      page.state.errors.categoryIds }}</p>
                  ...
                  <div>
                    ...
                    <div
                      ...
                      <TinyMCESelfHost v-model="page.state.formData.content" placeholder="Nhập mô tả sản phẩm"
                        :height="300" />
                    </div>
                    <p v-if="page.state.errors.content" class="mt-1 text-sm text-red-600">{{ page.state.errors.content
                    }}</p>
                  </div>
                </form>
              </div>
              ...
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                <div class="flex flex-col md:flex-row gap-4">
                  <div class="flex-1">
                    ...
                    <div class="relative">
                      <BaseNumberInput id="price" v-model="page.state.formData.price" :allow-decimal="false"
                        group-separator="," decimal-separator="." placeholder="0"
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="ml-1" title="Giá so sánh là giá gốc trước khi giảm giá."><svg
                          xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 inline" fill="none"
                    ...
                    <div class="relative">
                      <BaseNumberInput id="compareAtPrice" v-model="page.state.formData.compareAtPrice"
                        :allow-decimal="false" group-separator="," decimal-separator="." placeholder="0"
                        class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-12" />
                      <span
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-base pointer-events-none">đ</span>
                    </div>
                ...
                <div class="flex items-center mt-4">
                  <CustomCheckbox class="mr-2" :model-value="!!page.state.formData.chargeTax"
                    @update:model-value="page.state.formData.chargeTax = $event"><span
                      class="text-base text-gray-900 select-none">Tính thuế cho sản phẩm này</span></CustomCheckbox>
                </div>
                <div class="my-4 border-t border-gray-200" />
                <div class="flex flex-col md:flex-row gap-4">
                  <div class="flex-1">
                    <div class="flex items-center justify-between"><label
                        class="block text-sm font-medium text-gray-700 mb-1">Giá nhập</label><span class="ml-1"
                        title="Giá nhập là giá bạn mua vào."><svg xmlns="http://www.w3.org/2000/svg"
                          class="h-4 w-4 text-gray-400 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" />
                        </svg></span></div>
                    <div class="relative">
                      <BaseNumberInput id="importPrice" v-model="page.state.formData.importPrice" :allow-decimal="false"
                        group-separator="," decimal-separator="." placeholder="0"
                        class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-12" />
                      <span
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-base pointer-events-none">đ</span>
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between"><label
                        class="block text-sm font-medium text-gray-700 mb-1">Lợi nhuận</label><span
                        class="text-gray-500 text-xs" title="Lợi nhuận = Giá bán - Giá nhập"><svg
                          xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 inline" fill="none"
                          viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z" />
                        </svg></span></div>
                    <div class="relative"><input id="profit" :value="page.inventory.profitDisplay" readonly
                        class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-12"><span
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-base pointer-events-none">đ</span>
                    </div>
                  </div>
                  <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tỉ suất lợi nhuận</label>
                    <div class="relative"><input id="margin" :value="page.inventory.marginDisplay" readonly
                        class="w-full px-3 h-[36px] text-base rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 pr-8"><span
                ...
                <div class="space-y-4">
                  <div class="pt-1">
                    <CustomCheckbox :model-value="!!page.state.formData.manageInventory"
                      @update:model-value="page.state.formData.manageInventory = $event"><span
                        class="text-gray-900">Quản lý số lượng tồn kho</span></CustomCheckbox>
                  </div>
                  <div class="pt-1">
                    <div class="flex items-center justify-between mb-2">
                      <div class="text-sm text-gray-700 dark:text-gray-300 font-medium">Số lượng theo kho</div>
                      <NuxtLink to="#" class="text-sm text-primary-600 hover:underline">Chỉnh sửa kho</NuxtLink>
                    </div>
                    ...
                    <div v-for="w in page.inventory.warehouses" :key="w.id" class="grid grid-cols-2 gap-2 py-1">
                      ...
                      <div class="text-right flex items-center justify-end h-9">
                        <template v-if="page.state.formData.manageInventory">
                          <input
                            :value="(page.state.formData.warehouseStocks && page.state.formData.warehouseStocks[w.id]) ?? 0"
                            type="number" min="0"
                            ...
                            @input="(e: Event) => { const v = Number((e.target as HTMLInputElement).value || 0); const id = w.id; const next = { ...(page.state.formData.warehouseStocks || {}) }; next[id] = v; page.state.formData.warehouseStocks = next }">
                        </template>
                        <template v-else><span class="text-sm text-gray-500 inline-flex items-center h-9">Không theo
                            dõi</span></template>
                      </div>
                    </div>
                  </div>
                  <div v-if="page.state.formData.manageInventory">
                    <CustomCheckbox class="items-start" :model-value="!!page.state.formData.allowNegativeStock"
                      @update:model-value="page.state.formData.allowNegativeStock = $event"><span
                        class="text-gray-900">Cho phép bán khi
                        hết hàng<div v-if="page.state.formData.allowNegativeStock" class="text-gray-500 text-xs">Nhân
                          viên vẫn có thể bán
                          khi tồn kho bằng 0 hoặc âm. (Không ảnh hưởng đến POS)</div></span></CustomCheckbox>
                  </div>
                  <div class="space-y-2">
                    <CustomCheckbox :model-value="!!page.state.formData.hasSkuOrBarcode"
                      @update:model-value="page.state.formData.hasSkuOrBarcode = $event"><span class="text-gray-900">Sản
                        phẩm có mã SKU
                        hoặc mã vạch</span></CustomCheckbox>
                    <div v-if="page.state.formData.hasSkuOrBarcode" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div><label class="block text-sm font-medium text-gray-700 mb-1">Mã SKU (Stock Keeping
                          Unit)</label><input v-model="page.state.formData.sku" type="text"
                          class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500"
                          placeholder="Nhập mã SKU"></div>
                      <div><label class="block text-sm font-medium text-gray-700 mb-1">Mã vạch (ISBN, UPC, GTIN,
                          ...)</label><input v-model="page.state.formData.barcode" type="text"
                          class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500"
                          placeholder="Nhập mã vạch"></div>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div v-if="!page.media.imagePreviews.length"
                      ...
                      <div class="flex items-center justify-center gap-2 mb-2">
                        ...
                        <button type="button" class="ml-2 text-gray-700 hover:underline text-sm font-medium">Select
                          existing</button>
                      </div>
                    ...
                    <div v-else class="grid grid-cols-5 gap-3">
                      <template v-for="(img, idx) in page.media.imagePreviews" :key="idx">
                        <div
                          :class="['relative flex items-center justify-center border border-gray-300 rounded-md bg-white overflow-hidden', idx === 0 ? 'row-span-2 col-span-2 h-52 w-full' : 'h-24 w-full']"
                          draggable="true" @dragstart="(e) => page.media.onDragStart(e, idx)"
                          @dragover.prevent="page.media.onDragOver" @drop="(e) => page.media.onDrop(e, idx)"
                          @dragend="page.media.onDragEnd">
                          <img v-if="!page.media.isUploadingImage" :src="page.media.getPreviewUrl(img)"
                          ...
                          <div v-else-if="page.media.isUploadingImage"
                            ...
                            </svg><span class="text-gray-500 text-sm mt-2">Uploading...</span>
                          </div>
                          <button v-if="!page.media.isUploadingImage" type="button"
                            ...
                            @click="page.media.removeImage(idx)"><svg xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div
                        ...
                        @click="$refs.fileInput && ($refs.fileInput as HTMLInputElement).click()"
                        @dragover.prevent="page.media.onDragOver"
                        @drop="(e) => page.media.onDrop(e, page.media.imagePreviews.length)"><span
                          class="text-2xl text-gray-400">+</span></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <UPageCard variant="soft" class="bg-white rounded-lg">
              <div class="flex items-center justify-between">
                <div class="font-semibold text-base">Thuộc tính</div><button v-if="!page.variants.showAttributesEditor"
                  type="button" class="text-primary-600 hover:underline" @click="page.variants.startAttributes">Thêm
                  thuộc
                  tính</button>
              </div>
              <div v-if="!page.variants.showAttributesEditor"
                ...
                <p class="text-gray-600 w-full">Sản phẩm có nhiều thuộc tính khác nhau. Ví dụ: kích thước, màu sắc.</p>
              </div>
              <div v-else class="-mx-6 px-6 pt-2 pb-4 border-t-1 border-gray-200 dark:border-gray-700">
                <div class="grid grid-cols-2 gap-4 items-center mb-2">
                  <div class="font-medium text-gray-700">Tên thuộc tính</div>
                  <div class="font-medium text-gray-700">Giá trị</div>
                </div>
                <div v-for="(attr, idx) in page.variants.attributes" :key="idx"
                  ...
                  <div class="flex items-center gap-2">
                    <div class="flex flex-wrap gap-1 flex-1">
                      <span v-for="(val, vIdx) in attr.values" :key="vIdx"
                        class="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-sm">{{
                          val }}<button type="button" class="ml-1 text-blue-400 hover:text-blue-700"
                          @click="page.variants.removeAttrValue(idx, vIdx)">×</button></span>
                    ...
                    <button type="button" class="text-gray-400 hover:text-red-500"
                      @click="page.variants.removeAttribute(idx)"><svg xmlns="http://www.w3.org/2000/svg"
                ...
                <button type="button"
                  ...
                  :disabled="page.variants.attributes.length >= 3" @click="page.variants.addAttribute"><span
                    class="text-xl mr-1">+</span> Thêm thuộc tính khác</button>
              </div>
            </UPageCard>

            <!-- Variants -->
            <UPageCard v-if="page.variants.variants && page.variants.variants.length" title="Phiên bản" variant="soft"
              ...
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                ...
                <div class="flex flex-wrap items-center gap-3 mb-3"><span class="text-sm text-gray-700">Bộ
                    lọc:</span><template v-for="(a, i) in page.variants.activeAttributes" :key="i"><button type="button"
                      class="inline-flex items-center gap-1 text-sm text-primary-600 bg-primary-50 px-2 py-1 rounded-md hover:underline">{{
                        a.name }}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                ...
                <div class="flex items-center gap-2 py-2 border-y border-gray-200">
                  ...
                  <span class="font-medium flex items-center gap-1">
                    <template v-if="page.variants.selectedVariantKeys.length === 0">{{ page.variants.variants.length }}
                      phiên bản</template>
                  ...
                  <div v-for="(v, vi) in page.variants.variants" :key="v.key || vi"
                    ...
                    <div class="text-right text-sm leading-5">
                      <div>
                        ...
                        <span v-if="!page.variants.isEditingPrice(v.key)"
                        ...
                        <template v-else-if="page.variants.isEditingPrice(v.key)"><input
                            ref="el => priceInputs[vi] = el as HTMLInputElement" type="number" min="0"
                            ...
                            @keydown.enter.prevent="page.variants.commitPrice(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))"></template>
                      </div>
                      <div class="mt-1">
                        <span class="text-gray-500">Có thể bán</span>
                        <span v-if="!page.variants.isEditingStock(v.key)"
                          class="mx-1 underline cursor-pointer text-gray-700"
                          @click="page.variants.startEditStock(v.key); $nextTick(() => stockInputs[vi]?.focus())">{{
                            v.stock || 0
                          }}</span>
                        <template v-else-if="page.variants.isEditingStock(v.key)"><input
                            ref="el => stockInputs[vi] = el as HTMLInputElement" type="number" min="0"
                            ...
                            @keydown.enter.prevent="page.variants.commitStock(v.key, page.state.formData.warehouseId || 0, Number(($event.target as HTMLInputElement).value || 0))"></template>
                        <span class="text-gray-500">tại {{page.inventory.warehouses.find(w => w.id ===
                          page.state.formData.warehouseId)?.name || 'kho'}}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="w-full flex justify-end mt-8 mb-8">
              <UButton :label="isEdit ? 'Lưu sản phẩm' : 'Thêm sản phẩm'" color="primary" size="lg"
                :loading="page.state.productForm.isSubmitting" class="px-8 py-2 text-lg font-semibold rounded-md shadow"
                @click="onSubmit" />
            </div>
          </div>
          ...
          <div class="w-full lg:w-80 space-y-6">
            <UPageCard title="Cài đặt" variant="soft" class="bg-white rounded-lg">
              <div class="-mx-6 px-6 pt-4 border-t-1 border-gray-200 dark:border-gray-700">
                <div class="space-y-4">
                  ...
                  <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Giao diện trang sản
                      phẩm</label>
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      ...
                      <div class="text-sm text-gray-800 dark:text-gray-200">Cửa hàng trực tuyến <span
                          class="text-gray-500">đã
                          bán</span></div>
                    </div>
            ...
            <UPageCard variant="soft" class="bg-white rounded-lg">
              <template #title>
                <div class="flex items-center gap-1">Tổ chức sản phẩm <span
                    title="Thông tin về cách tổ chức sản phẩm"><svg xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <BaseDropdownSelect v-model="page.state.selectedSupplier"
                      :options="(page.state.suppliers || []).map(s => ({ id: s.id, label: s.name }))"
                      :loading="page.state.suppliersLoading" placeholder="Chọn nhà cung cấp"
                  ...
                  <div>
                    ...
                    <BaseDropdownSelect v-model="page.state.selectedCollections"
                      ...
                      class="w-full px-3 h-[36px] text-sm rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                    <div v-if="page.state.selectedCollections && page.state.selectedCollections.length > 0"
                      class="flex flex-wrap gap-1 mt-2">
                      <span
                        v-for="col in (page.state.collections || []).filter(c => page.state.selectedCollections.includes(c.id))"
                        :key="col.id"
                        class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200">{{
                          col.name }}<button type="button"
                          class="ml-1 inline-flex items-center justify-center w-3 h-3 rounded-full text-primary-600 hover:bg-primary-200 hover:text-primary-800 focus:outline-none"
                          @click="page.state.selectedCollections = page.state.selectedCollections.filter(id => id !== col.id)">×</button></span>
                    </div>
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tags</label>
                    <div class="w-full">
                      <input v-model="page.state.tagInput" type="text"
                      ...
                      <div v-if="page.state.tagsList.length" class="flex flex-wrap gap-2 mt-2">
                        <span v-for="(tag, idx) in page.state.tagsList" :key="`${tag}-${idx}`"
                          class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-normal bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200">{{
                            ...
                            @click="page.tags.removeTag(idx)">×</button></span>
                      </div>
                    </div>
                  </div>
                </div>
