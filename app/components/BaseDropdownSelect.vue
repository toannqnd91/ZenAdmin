...
<template>
    <div ref="dropdownRef" class="relative">
        <div :class="[props.class, 'flex items-center cursor-pointer', { 'opacity-50 pointer-events-none': props.loading }]"
            @click="isOpen = !isOpen">
        ...
        <div v-if="isOpen" class="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg">
            ...
            <div v-if="props.addNewLabel && search && !filteredOptions.length"
                class="px-3 py-2 cursor-pointer hover:bg-gray-100 flex items-center gap-2" @click="handleAddNew">
            ...
            <div v-else>
                <div v-for="opt in filteredOptions" :key="opt.id"
                    ...
                    <input v-if="props.multiple" type="checkbox" :checked="isSelected(opt.id)"
                        @change.stop="toggleOption(opt.id)">
